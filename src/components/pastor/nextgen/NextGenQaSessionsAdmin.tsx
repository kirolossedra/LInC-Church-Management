      ...
      {expanded && <div className="space-y-8 p-6 md:p-8">
        ...
        {loading ? <div className="flex justify-center py-12"><Loader2 className="animate-spin" /></div> : sessions.length === 0 ? <p className="rounded-3xl border border-dashed border-stone-300 p-12 text-center text-stone-500">Create the first QA session above.</p> : <>
          ...
          {detail && <div className="space-y-7">
            ...
            <form onSubmit={addQuestion} className="grid gap-4 rounded-3xl border border-[#a66c18]/20 bg-[#fffaf0] p-6 lg:grid-cols-2"><div><label className="text-xs font-black uppercase tracking-widest text-stone-500">Question</label><textarea value={prompt} onChange={event => setPrompt(event.target.value)} required rows={5} className="mt-2 w-full rounded-2xl border border-stone-200 bg-white p-4" /></div><div><label className="text-xs font-black uppercase tracking-widest text-stone-500">Answer options · one per line</label><textarea value={optionsText} onChange={event => setOptionsText(event.target.value)} required rows={5} className="mt-2 w-full rounded-2xl border border-stone-200 bg-white p-4" /><button disabled={busy} className="mt-3 rounded-xl bg-[#a66c18] px-5 py-3 font-black text-white">Add question</button></div></form>
