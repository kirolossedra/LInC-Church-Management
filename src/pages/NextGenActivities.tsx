...
interface SavedBibleVerseEntry {
  reference: string;
  text: string;
}
...
function normalizeSavedSession(firebaseId: string, value: any): SavedQASession {
  ...
  const verses = Array.isArray(value?.verses)
    ? value.verses
        ...
        .filter((verse: SavedBibleVerseEntry) => verse.reference || verse.text)
    : [];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            ...
            <div className="p-7 rounded-[28px] bg-white border border-[rgba(139,30,30,0.08)] shadow-sm">
              ...
              <p className="text-[#666] leading-relaxed">
                {isArabic
                  ? 'هذه الصفحة تستخدم ترجمة WEB العامة للوصول إلى الآيات، وتحفظ الأسئلة مع حقول تصويت غير ظاهرة كمدخلات للمستخدم.'
                  : 'This page uses the public-domain WEB translation and saves each question with hidden vote counters for later review workflows.'}
              </p>
            </div>
          </div>
              ...
              <div className="p-6 md:p-8 space-y-6">
                ...
                {!isLoadingPeerReview && activePeerReviewSession && (
                  ...
                  >
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          ...
                          <span className="px-3 py-1 bg-white text-gray-500 rounded-full text-xs font-bold border border-gray-100">
                            {activePeerReviewSession.translation}
                          </span>
                        </div>
                    ...
                    {activePeerReviewSession.verses.length > 0 && (
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                          {isArabic ? 'الآيات المرتبطة' : 'Related Verses'}
                        </h4>

                        {activePeerReviewSession.verses.map((verse, index) => (
                          <div key={`${activePeerReviewSession.firebaseId}-verse-${index}`} className="bg-white border border-gray-100 rounded-2xl p-4">
                            {verse.reference && (
                              <div className="text-sm font-bold text-[#8b1e1e] mb-2">
                                {verse.reference}
                              </div>
                            )}
                            {verse.text && (
